...
auto execFunc(Func func, Args ... args)
{
  using namespace fu::detail;

  int x = 0;
  typename std::result_of<decltype(func)(argument_type_of_t<Func>...)>::type res;
  res = func(args...);
  return res;
}
...
int main()
{
  //execFunc(&dummyFunc, 5);
  std::cout << execFunc(&dummyFunc, 5) << std::endl;
  //std::result_of<decltype(&Dummy::Method)(Dummy, int)>::type var = 4;
